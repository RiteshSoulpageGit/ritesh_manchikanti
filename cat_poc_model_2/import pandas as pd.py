...
import re
import torch
from transformers import BigBirdTokenizer, BigBirdModel, BigBirdForSequenceClassification, AutoTokenizer, AutoModelForSequenceClassification
...
from transformers import get_linear_schedule_with_warmup, BigBirdConfig
import warnings
...
class Trainer:
    def __init__(self,model,tokenizer, rank, save_every,model_save_path):
        self.gpu_id = gpu_id
...
def load_train_objs():
    # can update this parameter to model path :path
...
def main(rank, world_size,path,model_save_path):
    ...
    chunks,model,tokenizer = load_train_objs()
    print("The total number of chunks in the document is ",len(chunks))
    trainer = Trainer(model,tokenizer, rank, save_every,model_save_path)
